			...
			<form method="post" action="/update_tmptidur">
				...
				<div class="mb-3">
					...
					<select name="status" id="status" class="form-control">
						<option value="Terisi" <?php if ($data->status == 'Terisi') {
													echo 'selected';
												}  ?>>Terisi</option>
						<option value="Kosong" <?php if ($data->status == 'Kosong') {
													echo 'selected';
												} ?>>Kosong</option>
					</select>
				</div>
				...
				<button type="submit" class="btn" style="background-color:#8caca2;" name="update">Update</button>
			</form>
		</div>
		<!--/div-->
	</div>
