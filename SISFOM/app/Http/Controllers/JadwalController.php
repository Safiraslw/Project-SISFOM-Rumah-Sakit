...
class JadwalController extends Controller
{
    ...
    public function store_jadwal(Request $request)
    {
        ...
        if ($dokter == null) {
            return redirect('/tambah_jadwal')->with('empty', 'Data dokter tidak ditemukan. 
            Mohon periksa nomor dokter kembali');
        }
    ...
    {
        $jadwal = DB::table('dokter')->where('nama', 'like', "%$request->keyword%")->join('jadwal', 
        'dokter.id', '=', 'jadwal.id_dokter')->select('jadwal.*', 'dokter.nama')->get();
